...
class _DoctorChangePasswordScreenState extends State<DoctorChangePasswordScreen> {
  ...
  Future<void> _changePassword() async {
    ...
    if (next.length < 6) { _showError('New password must be at least 6 characters'); return; }
    ...
    try {
      ...
      if (mounted) {
        await showDoctorNotice(context, 'Password changed successfully');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          DoctorBackHeader(title: 'Change Password', onBack: () => Navigator.pop(context)),
          Expanded(
  ...
  Widget _buildStep2() {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 8),
        TextField(controller: _newController, obscureText: true, decoration: DS.inputDecoration(hintText: 'At least 6 characters')),
        const SizedBox(height: 16),
